...
#ROOT_FOLDER=/data/maldonado/Data/tagc-mimicintw/jobs
...
do
	...
	#source ~/.snakemake/bin/activate
        #echo "Snakemake version=" $(snakemake --version) >  log_version.txt
	echo "Job_folder=" $job_folder >> log_version.txt
	echo "Current folder=" $(pwd) >> log_version.txt

#        docker exec mimicint_slurm_snakemake /bin/bash -c "cd $job_folder && snakemake --snakefile $job_folder/workflow/mimicint_snakefile \
#                  --jobs 2 \
#                  --reason \
